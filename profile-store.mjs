...
export function createProfileStore(db,fail){
 ...
 function cleanResult(r){
  ...
  if(r.content==='typing' && r.details?.typingStats?.version===1){
   const source=r.details.typingStats,stats={version:1};
   for(const [key,max] of Object.entries({strokes:1000000,elapsedMs:7200000,submissions:50000,characters:5000000,errors:5000000}))stats[key]=Math.max(0,Math.min(max,Math.floor(Number(source[key])||0)));
   stats.errors=Math.min(stats.characters,stats.errors);
   stats.cpm=stats.elapsedMs?Math.round(stats.strokes*60000/stats.elapsedMs):0;
   stats.accuracy=stats.characters?Math.round((stats.characters-stats.errors)/stats.characters*1000)/10:null;
   out.details.typingStats=stats;
   if(stats.accuracy!==null)out.details.accuracy=stats.accuracy;else delete out.details.accuracy;
  }
