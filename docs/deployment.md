...
이 문서는 `https://dwingul.com` 공개 배포를 위한 실행 순서와 아직 외부 서비스에서 끝내야 할 작업을 구분한다. 작성 기준일은 2026-09-14이며, 아래 외부 설정은 이 저장소 작업만으로 완료됐다고 보지 않는다.
...
다음 단계는 계정 권한이나 소유권 확인이 필요하다. 저장소의 코드와 정적 파일 생성만 완료된 상태에서는 끝난 것으로 표시하지 않는다.
...
1. Pull Request나 기본 브랜치 갱신 뒤 CI가 Node.js 24에서 `npm ci`, `npm test`, `npm run build`, `npm run check`를 모두 통과하는지 확인한다.
