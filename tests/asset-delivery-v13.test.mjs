...
test('critical assets contain only selected car rears and required gameplay sprites',()=>{
 const cars=gameAssetSources('racing',{car:'rally'});assert.ok(cars.includes('/assets/pixel/scenes/player-rally-rear.png'));assert.ok(cars.includes('/assets/pixel/scenes/traffic-bus-rear.png'));assert.ok(!cars.some(s=>/traffic-.*-(front|left|right)/.test(s)));assert.ok(!cars.includes('/assets/pixel/scenes/player-sport-rear.png'));const jump=gameAssetSources('jump');assert.ok(jump.includes('/assets/pixel/scenes/jump-obstacle-middle.png'));assert.equal(jump.filter(s=>s.includes('jump-obstacle')).length,5);assert.ok(gameAssetSources('typing',{characterId:'owl-aviator',startStage:11}).some(s=>s.endsWith('duel-hero-15.png')));
});
