  ...
  try{for(const viewport of views){
   ...
   for(const [label,panel] of [['뽑기 상점','shop'],['내 장비','gear'],['캐릭터','characters'],['프로필 · 저장','profile']]){
    ...
    if(panel==='profile'){
     ...
     await p.locator('.rpg-save-card summary').click();await p.locator('[name=nickname]').fill('메뉴 검증');await p.locator('[name=pin]').fill('Test-2468!');if(await p.locator('[name=passwordConfirm]').count())await p.locator('[name=passwordConfirm]').fill('Test-2468!');
