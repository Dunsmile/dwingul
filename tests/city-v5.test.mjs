...
import {
  ...
  frontGap,
  vehicleLength,
} from "../public/js/legacy/city-engine-v5.js";
...
test("only a same-lane evasive change at a 1–2 m front gap awards one near miss", () => {
  ...
  early.state.vehicles = [vehicleAtGap(1, 3)];
  early.move(-1);
  early.tick(700);
  assert.equal(early.state.nearMisses, 0, "an early lane change outside two meters is not rewarded");

  const fast = quietEngine({ car: "sport" });
  fast.state.seconds = 30; fast.state.speed = 126; fast.state.boost = 10; fast.boost();
  fast.state.vehicles = [vehicleAtGap(1, 3)]; fast.move(1); fast.tick(12);
  assert.equal(fast.state.nearMisses, 0, "3 m is too early even if boosted travel crosses 2 m during the change");
...
test("coin and fuel pickups use their intended common and rare distance intervals", () => {
  ...
  assert.ok(engine.state.nextFuelAt >= 280 && engine.state.nextFuelAt < 360);

  const firstCoinAt = engine.state.nextCoinAt;
  engine.tick(4);
  ...
  assert.equal(engine.state.pickups.some(({ type }) => type === "fuel"), false);

  const firstFuelAt = engine.state.nextFuelAt;
  engine.state.distance = firstFuelAt - (CITY.horizon - 15);
  engine.tick(4);
  const fuelGap = engine.state.nextFuelAt - firstFuelAt;
  assert.ok(fuelGap >= 1100 && fuelGap < 1400);
  assert.ok(engine.state.pickups.some(({ type }) => type === "fuel"));
  assert.ok(fuelGap > coinGap * 4, "fuel pickups remain substantially rarer than coins");
});
