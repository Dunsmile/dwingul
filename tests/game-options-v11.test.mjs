...
test('current rhythm identity and v16 jump defaults and retain explicit supported legacy settings',()=>{
 ...
 assert.deepEqual(gameSettings('jump'),{version:'v16'});
 assert.equal(gameMode('jump',{}),'jump-distance-v16');
 for(const version of ['v13','v11','v6','v5','v4']){assert.deepEqual(gameSettings('jump',{version}),{version});assert.equal(gameMode('jump',{version}),`jump-distance-${version}`);}
...
test('unknown versions sanitize to current while known city history keeps its identity',()=>{
 ...
 assert.equal(gameSettings('jump',{version:'unknown'}).version,'v16');
