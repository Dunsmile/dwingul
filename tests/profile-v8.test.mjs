...
test('one profile preserves identity, private birth/history/bests, recovers without exposing or merging another user',async()=>{
 ...
 try{
  ...
  const created=await a('profile','POST',{nickname:'공통프로필',pin:'Test-2468!'});assert.equal(created.user.id,uid);assert.equal((await a('rpg')).configured,true);
  ...
  await b('profile','POST',{nickname:'다른사람',pin:'Test-1357!'});await b('profile/birth','PUT',{...birth,name:'타인',birthday:'20020503'});assert.deepEqual((await b('profile')).history,[]);
  ...
  const restored=await recovered('recover','POST',{recovery:created.recovery,pin:'Test-2468!'});assert.equal(restored.user.id,uid);const restoredState=await recovered('profile');assert.deepEqual(restoredState.birth,birth);assert.equal(restoredState.history.length,100);assert.equal((await recovered('garage')).unlocked.length,1);assert.equal((await recovered('rpg')).configured,true);
