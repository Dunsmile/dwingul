...
test('a recovered profile retains long-mode history and bests without input logs or equipment snapshots',async()=>{
 ...
 try{
  const user=client(),created=await user('profile',{nickname:'긴 문장 보관',pin:'Test-2468!'});
  ...
  const recovered=client();assert.equal((await recovered('recover',{recovery:created.recovery,pin:'Test-2468!'})).status,200);
...
test('a long run checkpoints, replays, settles and registers its authoritative long score',async()=>{
  ...
  try{
    const user=client();
    await user('profile',{nickname:'긴 문장 검증',pin:'Test-2468!'});
...
test('invalid and omitted sentence modes stay on the legacy short key while rankings remain separated',async()=>{
  ...
  try{
    const user=client();
    await user('profile',{nickname:'모드 분리 검증',pin:'Test-1357!'});
    const attempts=[];
...
test('long rooms and challenge shares preserve the mode with stage-one baseline gear',async()=>{
  ...
  try{
    ...
    await owner('profile',{nickname:'긴 문장 방장',pin:'Test-1234!'});
    await friend('profile',{nickname:'긴 문장 친구',pin:'Test-5678!'});
