...
test('portable API rejects cross-origin state changes before mutating the authenticated session', async () => {
  ...
  try {
    ...
    const rejected = await request('profile', 'POST', {nickname: '공격자', pin: 'Test-1234!'}, {origin: 'https://evil.example'});
    ...
    const accepted = await request('profile', 'POST', {nickname: '안전한 사용자', pin: 'Test-1234!'}, {origin: 'https://worker.test'});
...
test('portable profile and share APIs preserve validated 12-answer personality version data', async () => {
  ...
  try {
    ...
    await request('profile', 'POST', {nickname: '취향 탐험가', pin: 'Test-1234!'}, {origin: 'https://worker.test'});
...
test('portable API ranks current rhythm v11 with points and retains separate v9, v7 and nine-pad modes', async () => {
  ...
  try {
    const request = client(fixture.handler);
    const session = await request('session');
    await request('profile', 'POST', {nickname: '박자 수집가', pin: 'Test-1234!'}, {origin: 'https://worker.test'});
