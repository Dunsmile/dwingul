...
function injectHomeSeo(source) {
  ...
  const catalogLinks=catalog.map(item=>`<li><a href="/content/${item.id}/"><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.desc)}</span></a></li>`).join('');
