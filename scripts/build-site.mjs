...
export const guides = {
  ...
  typing:{
    ...
    rules:'문장을 입력하고 Enter로 공격합니다. 일반 몬스터는 10초마다, 10스테이지마다 나타나는 보스는 15초마다 반격합니다. MP 100에서 빈 입력창에 ‘힐’을 입력하면 기본 15 HP를 회복합니다. 25종 몬스터가 순환하며, 16종 캐릭터는 전투 수치가 같은 꾸미기·수집 요소입니다.',
  ...
  racing:{
    ...
    rules:'5차선에서 차량을 피하고 동전·연료를 모읍니다. 연료는 초당 0.5칸 줄고 아이템은 15칸을 채웁니다. 충돌은 기본 10칸에서 차량 장갑만큼 줄어든 연료 피해를 주며, 한 번 맞으면 0.9초 동안 보호됩니다. 연료가 0이 되면 주행이 끝납니다. 8종 차량은 속도·탱크·부스터 지속 시간·동전 충전 확률·장갑이 다릅니다.',
    tips:['항상 현재 차선 양옆 중 어느 쪽이 비었는지 미리 확인하세요.','연료 아이템은 +15칸이므로 탱크가 많이 비었을 때 경로에 넣으세요.','충돌하면 기본 10칸에서 장갑만큼 줄어든 피해를 받고 0.9초 보호되므로, 보호가 끝나기 전에 안전한 차선으로 빠져나오세요.'],
    scoring:'충돌은 연료를 줄이지만 즉시 끝내지 않으며, 연료가 바닥날 때까지 이동한 거리(m)가 기록입니다. 모은 동전, 충돌 횟수와 차량별 규칙도 결과 재현에 함께 사용합니다.',
    faq:[['연료는 얼마나 줄고 채워지나요?','연료는 초당 0.5칸 줄고 연료 아이템 하나가 15칸을 채웁니다. 충돌 피해도 연료에서 빠지며 탱크 최대치를 넘지는 않습니다.'],['부스터는 어떻게 쓰나요?','근접 회피와 차량별 동전 충전 확률로 10칸을 모은 뒤 위쪽 키나 화면의 부스터 버튼을 누르면 가속합니다. 차량마다 지속 시간이 다릅니다.'],['새 차량은 유료인가요?','현금 결제가 아니라 게임에서 모은 토큰을 씁니다. 기본 차량 외 차량마다 50~200토큰으로 정해진 가격이 다릅니다.']]},
  memory:{
...
export async function buildSite() {
  ...
  const sitemap=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">\n${urls.map(pathname=>`  <url><loc>${absolute(pathname)}</loc><lastmod>${buildDate}</lastmod>${catalog.some(item=>pathname===`/content/${item.id}/`)?`<image:image><image:loc>${absolute(`/assets/pixel/thumbnails/${pathname.split("/")[2]}.webp`)}</image:loc></image:image>`:""}</url>`).join('\n')}\n</urlset>\n`;
