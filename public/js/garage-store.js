...
export function createGarageStore(db, fail) {
  ...
  function requireCar(userId, car) { if (!wallet(userId).unlocked.includes(car)) fail('이 차량은 잠겨 있어요. 차고에서 50토큰으로 먼저 열어주세요.',403); }
  function unlock(userId, car) {
    ...
    return atomic(db,()=>{
      ...
      if (!w.unlocked.includes(car)) {
        if (w.tokens < spec.cost) fail('50토큰을 모으면 이 차량을 열 수 있어요.');
  ...
  function settle(userId, data) {
    ...
    if(!['v4','v5','v6'].includes(JSON.parse(attempt.game_settings).version))fail('이전 주행은 차고에서 새로 시작해주세요.');
    const settings = gameSettings('racing',JSON.parse(attempt.game_settings)), spec = cityCars.find(c=>c.id===settings.car);
    ...
    const replay=(JSON.parse(attempt.game_settings).version==='v4'?createCityV4:JSON.parse(attempt.game_settings).version==='v5'?createCityV5:createCityEngine)({car:spec.id,random:seededRandom(`racing:${attempt.seed}`)});
    let time=0;
