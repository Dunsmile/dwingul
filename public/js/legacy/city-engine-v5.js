...
export const CITY=Object.freeze({horizon:240,acceleration:1.08,baseSpeed:18,maxSpeed:42,playerHalfWidth:.27,playerHalfLength:1.8,carHalfWidth:.32,busHalfWidth:.36,laneSeconds:.012,evadeMin:1,evadeMax:2,fuelDrain:2,fuelPickup:15,boostDrain:2});
...
export function createCityEngine({car='basic',random=Math.random}={}){
 ...
 function step(dt){if(state.ended)return;const seconds=Math.min(dt,state.fuel/CITY.fuelDrain);state.seconds+=seconds;state.speed=citySpeed(spec,state.seconds,state.boosting);const distance=state.speed*seconds;state.distance+=distance;state.fuel=Math.max(0,state.fuel-2*seconds);state.flashTime=Math.max(0,state.flashTime-seconds);
  ...
  for(const candidate of state.evadeCandidates){const v=state.vehicles.find(v=>v.id===candidate.id);if(v&&!v.evaded&&state.lane===candidate.to&&Math.abs(state.x-candidate.to)<.02&&state.seconds<=candidate.expires&&frontGap(v)<=CITY.evadeMax)near(v,{...candidate,gap:Math.max(1,Math.min(2,frontGap(v)))} );}
  ...
  while(state.nextFuelAt-state.distance<=CITY.horizon-15){spawnPickup('fuel',state.nextFuelAt);state.nextFuelAt+=1100+random()*300;}
