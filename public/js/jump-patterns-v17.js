...
export const JUMP_V17_RULES = Object.freeze({
  ...JUMP_V16_RULES,
  initialSpeed:260,
  speedStep:28,
  maxSpeed:480,
  initialObstaclesPer10s:7,
  fastFallMinVy:560,
  fastFallGravity:2600,
  chainRecoveryDistance:Object.freeze({basic:460,slide:420,'slide-chain':400,'single-double':540,'jump-slide':500,'master-chain':680}),
});

export function jumpWorldSpeedV17(elapsedMs){return Math.min(JUMP_V17_RULES.maxSpeed,JUMP_V17_RULES.initialSpeed+jumpDifficultyLevel(elapsedMs)*JUMP_V17_RULES.speedStep);}
export function jumpV17TargetObstaclesPer10s(elapsedMs,distanceMeters){return (JUMP_V17_RULES.initialObstaclesPer10s+jumpDifficultyLevel(elapsedMs))*jumpStage(distanceMeters).frequencyScale;}

function chainPattern(id,type,minStage,gapAfterDistance,events,minElapsedMs=0){return Object.freeze({id,type,minStage,minElapsedMs,variant:id,gapAfterDistance,events:Object.freeze(events)});}
const slideChain=(count,index)=>chainPattern(`slide-chain-${count}-${index+1}`,'slide-chain',Math.max(0,count-1),330+index*10,Array.from({length:count},(_,i)=>event('slide',i*(142+index*4),104+(index%4)*6,348+(index%3))));
const dash=index=>chainPattern(`dash-${index+1}`,'basic',0,320+index*8,[event(index%3===2?'wide':'ground',0,index%3===2?98+(index%4)*6:48+(index%5)*5,index%3===2?30:50+(index%3)*3)]);
const singleDouble=index=>chainPattern(`single-double-${index+1}`,'single-double',index<3?0:2,500+index*10,[event('ground',0,48+(index%4)*4,50),event('double',125+(index%5)*8,50+(index%3)*4,148+(index%4)*4)],index<3?10000:0);
const jumpSlide=index=>chainPattern(`jump-slide-${index+1}`,'jump-slide',index<3?1:3,450+index*10,[event('ground',0,48+(index%4)*5,52),event('slide',330+(index%5)*12,108+(index%4)*6,348+(index%3))]);
const masterChain=index=>chainPattern(`master-chain-${index+1}`,'master-chain',5,640+index*12,[event('ground',0,48,52),event('double',130+(index%3)*8,52,150+(index%4)*3),event('slide',520+(index%4)*14,110+(index%3)*8,349)]);

export const jumpPatternsV17=Object.freeze([
  ...Array.from({length:15},(_,i)=>dash(i)),
  ...Array.from({length:8},(_,i)=>slideChain(1,i)),
  ...Array.from({length:8},(_,i)=>slideChain(2,i)),
  ...Array.from({length:8},(_,i)=>slideChain(3,i)),
  ...Array.from({length:8},(_,i)=>slideChain(4,i)),
  ...Array.from({length:8},(_,i)=>slideChain(5,i)),
  ...Array.from({length:15},(_,i)=>singleDouble(i)),
  ...Array.from({length:15},(_,i)=>jumpSlide(i)),
  ...Array.from({length:15},(_,i)=>masterChain(i)),
]);

export function jumpV17PatternsForStage(stageIndex,elapsedMs=Infinity){
  const stage=Math.max(0,Math.min(5,Number(stageIndex)||0));
  return jumpPatternsV17.filter(pattern=>pattern.minStage<=stage&&pattern.minElapsedMs<=elapsedMs&&(
    stage>=4||pattern.events.length<=stage+2
  ));
}

export function shuffledJumpPatternsV17(random,stageIndex,elapsedMs=Infinity){
  const deck=[...jumpV17PatternsForStage(stageIndex,elapsedMs)];
...
export function jumpV17NextPatternDistance(pattern,elapsedMs,distanceMeters){
  const speed=jumpWorldSpeedV17(elapsedMs),target=jumpV17TargetObstaclesPer10s(elapsedMs,distanceMeters);
  const last=Math.max(0,...pattern.events.map(item=>item.atDistance));
  const desired=speed*10/target*pattern.events.length;
  return last+Math.max(JUMP_V17_RULES.chainRecoveryDistance[pattern.type]||340,desired-last);
}
