...
export function createRpgStore(db,fail){
 db.exec(`CREATE TABLE IF NOT EXISTS rpg_profiles(user_id TEXT PRIMARY KEY REFERENCES users(id),gold INTEGER NOT NULL DEFAULT 0,best_cleared INTEGER NOT NULL DEFAULT 0,equipped TEXT NOT NULL DEFAULT '{}');
 ...
 function progress(user,data){const attempt=get('SELECT * FROM runs WHERE id=? AND user_id=?',data.run,user.id);if(!attempt||attempt.content!=='typing')fail('진행 중인 모험을 찾을 수 없어요.');const config=JSON.parse(attempt.game_settings||'{}');if(config.version!=='v5')fail('타이핑 마스터를 새로 시작해주세요.');
  ...
  const model=createTypingRpgModel({random:seededRandom(`typing:${attempt.seed}`),startStage:config.startStage,gear:config.gear});let time=0;
 ...
 return{profile,settings,draw,equip,enhance,discard,progress};
}
