...
export async function openRpgDraw({draw,equip,onResult,onReveal=()=>{},stored=false}) {
 ...
 body.innerHTML=`<div class="draw-kicker">나의 다음 모험</div><h2 id="draw-title">두근두근,<br>상자를 준비하고 있어요!</h2>${chest('sealed','is-opening')}<p>어떤 장비가 기다리고 있을까요?</p><div class="draw-footer">상자 안의 반짝임을 모으는 중…</div>`;
 wireChestFallback(body);
 modal.showModal();
 ...
 function showReady(){
  ...
  modal.className='rpg-draw-dialog';
  body.innerHTML=`<div class="draw-kicker">${total>1?`${number} / ${total}`:'준비됐어요!'}</div><h2 id="draw-title">이번 상자에는…?</h2><button type="button" class="draw-reveal">${chest('sealed','is-ready')}<strong>터치해서 결과보기</strong></button><p>${total>1?'한 개씩 차례로 확인해요.':'상자를 눌러 나의 장비를 확인하세요.'}</p>`;
  wireChestFallback(body);
