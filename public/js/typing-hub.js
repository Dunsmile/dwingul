...
export function typingHub(profile,params,lastDraw,profileFields='',startControl=''){
 ...
 const setupButton=`<a class="button primary full rpg-start" href="${route(params,{panel:'settings'})}">모험 시작하기 →</a>`;
 const links=[['shop','뽑기 상점'],['gear','내 장비'],['characters','캐릭터'],['profile','프로필 · 저장']];
 let body='';
 if(panel==='menu')body=`<div class="rpg-title-screen"><div class="rpg-title-copy"><span>DWINGUL · FOREST ADVENTURE</span><h1>타이핑 마스터</h1><p>한 문장으로 시작하는 숲속 모험</p></div><nav class="rpg-title-buttons" aria-label="타이핑 마스터 메뉴">${setupButton}${links.map(([id,name])=>`<a class="button rpg-menu-button" href="${route(params,{panel:id})}">${name}</a>`).join('')}</nav><div class="rpg-title-current"><strong>${modeName}</strong><span>${start}스테이지부터 · ${esc(selected.name)}</span></div></div>`;
