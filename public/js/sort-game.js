...
export function createSortGame(ctx){
 const model=createSortModel({random:ctx.random,mode:ctx.settings.mode}),s=model.state,canvas=document.createElement('canvas');canvas.width=900;canvas.height=500;canvas.className='dg-game__canvas';canvas.setAttribute('aria-label','앞에 멈춰 있는 캐릭터를 파랑은 왼쪽, 하양은 오른쪽으로 보내세요. 피버는 어느 쪽이든 가능해요.');const g=canvas.getContext('2d');preloadWorld(['blue','white','purple'].flatMap(c=>['normal','smile','cry'].map(f=>`sort-${c}-${f}`)));
 ...
 function character(item,x,y,size,face='normal',purple=false){const name=`sort-${purple?'purple':item.side==='left'?'blue':'white'}-${face}`;if(!drawWorldSprite(g,name,x-size/2,y-size,size)){g.fillStyle=purple?'#a182be':item.side==='left'?'#76abd0':'#fff4db';g.fillRect(x-size/2,y-size,size,size);}}
 function draw(){const H=canvas.height,W=canvas.width,C=W/2,purple=s.feverMs>0;g.fillStyle=purple?'#e5daeb':'#e9edda';g.fillRect(0,0,W,H);g.fillStyle=purple?'#a994b3':'#bda37d';g.beginPath();g.moveTo(W*.433,50);g.lineTo(W*.567,50);g.lineTo(W*.867,H);g.lineTo(W*.133,H);g.closePath();g.fill();g.strokeStyle='#7e694e';g.lineWidth=4;for(let n=0;n<5;n++){const y=H*.23+n*n*H*.028;g.beginPath();g.moveTo(C-(y/H)*W*.367,y);g.lineTo(C+(y/H)*W*.367,y);g.stroke();}
  ...
  g.font='bold 26px Galmuri11, sans-serif';g.textAlign='center';g.fillStyle=purple?'#684298':'#456343';g.fillText(s.phase!=='playing'?(s.endReason==='time'?'시간 끝! 잘했어요 ^^':'잠깐 쉬어가요 ㅠㅠ'):purple?'피버! 어느 쪽이든 OK':s.streak>1?`${s.streak} 연속!`:'1.2초 안에 이어 보내면 피버 +5',C,36);
